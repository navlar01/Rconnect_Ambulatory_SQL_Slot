...
SELECT d.* FROM(
    SELECT c.*, ROW_NUMBER() OVER (PARTITION BY DEPARTMENT_ID, PROV_ID, APPT_DTTM, OVERBOOKED_YN ORDER BY DEPARTMENT_ID, PROV_ID, APPT_DTTM, OVERBOOKED_YN) AS ROW_ID 
        FROM (
            Select a.* FROM(
                (SELECT * FROM( 
                  SELECT PAT_ENC_CSN_ID, SITE AS CAMPUS, DEPARTMENT_NAME, DEPT_SPECIALTY_NAME,
                   DEPARTMENT_ID, PROV_NAME_WID, PROV_ID, NPI,VISIT_PROV_STAFF_RESOURCE_C,
...
FROM(
    (SELECT * FROM AMBULATORY_ACCESS_RAW_FILTERED WHERE (APPT_STATUS IN ('Arrived', 'No Show')) OR 
...
FROM(
    ...
    RIGHT JOIN
    (SELECT * FROM(
          (SELECT DISTINCT DEPARTMENT_NAME, PROV_ID FROM AMBULATORY_ACCESS_RAW_FILTERED))) b
...
slot_table_quary <- glue("CREATE TABLE AMBULATORY_SLOT_TABLE AS
                            ...
                            FROM
                            (
                              SELECT e.CAMPUS, e.DEPARTMENT_ID, e.DEPARTMENT, e.CAMPUS_SPECIALTY, 
                              c.PROV_ID, TRIM(TRAILING FROM REGEXP_REPLACE(PROV_NM_WID, '\\[(.*?)\\]', '')) AS                                 Provider, 
                              c.SLOT_DATE, c.SLOT_MONTH_YEAR, SLOT_DATE - (TO_CHAR(SLOT_DATE, 'D') - 1) AS    
                              APPT_WEEK, 
                              c.AVAILABLE_HOURS, c.SCHEDULED_HOURS, c.ARRIVED_HOURS, TO_CHAR(c.SLOT_DATE, 'DY') 
                              AS APPT_DAY,
                              a.DEPT_SPECIALTY_NAME, a.NPI, a.VISIT_PROV_STAFF_RESOURCE_C 
                              FROM
                              (
                                ...
                                (
                                  ...
                                  FROM AMBULATORY_ACCESS_RAW_FILTERED
                                ) a
                                LEFT JOIN
                                (
                                  SELECT * FROM AMBULATORY_ACCESS_SCHED_ARRIVED_HOURS
                                ) b
                                ON a.DEPARTMENT_ID = b.DEPARTMENT_ID AND a.PROV_ID = b.PROV_ID AND a.SLOT_DATE = 
                                b.APPT_DATE
                              ) c
                              LEFT JOIN
                              (
                            ...
                            ON f.DEPARTMENT_ID = h.DEPARTMENT_ID AND f.PROV_ID = h.PROV_ID AND f.SLOT_DATE = 
                            h.SLOT_DATE
                            LEFT JOIN
                            (
                              SELECT DEPARTMENT_ID, PROV_ID, SLOT_DATE, 
                            ...
                            ON f.DEPARTMENT_ID = i.DEPARTMENT_ID AND f.PROV_ID = i.PROV_ID AND f.SLOT_DATE = 
                            i.SLOT_DATE
                            LEFT JOIN
                            (
                              SELECT DEPARTMENT_ID, PROV_ID, SLOT_DATE, 
                            ...
                            ON f.DEPARTMENT_ID = j.DEPARTMENT_ID AND f.PROV_ID = j.PROV_ID AND f.SLOT_DATE = 
                            j.SLOT_DATE
                            LEFT JOIN
                            (
                              SELECT DEPARTMENT_ID, PROV_ID, SLOT_DATE, 
                            ...
                            ON f.DEPARTMENT_ID = k.DEPARTMENT_ID AND f.PROV_ID = k.PROV_ID AND f.SLOT_DATE = 
                            k.SLOT_DATE
                            ")
