...
### Processing Unavailable Hours, Held Hours, Private Hours and Overbooked Hours together
unavail_hours_query <- glue("CREATE TABLE AMBULATORY_SLOT_TABLE AS
SELECT f.*, g.holiday, h.UNAVAILABLE_HOURS, i.HELD_HOURS, j.PRIVATE_HOURS, k.OVERBOOKED_HOURS
FROM
  ...
  FROM
          ...
          FROM
            (SELECT * FROM AMBULATORY_AVAILABILITY_RAW_FILTERED) a
          LEFT JOIN
            (SELECT * FROM AMBULATORY_ACCESS_SCHED_ARRIVED_HOURS) b
          ON a.DEPARTMENT_ID = b.DEPARTMENT_ID AND a.PROV_ID = b.PROV_ID AND a.SLOT_DATE = b.APPT_DATE) c
  LEFT JOIN
    (SELECT DISTINCT CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT_ID, DEPARTMENT 
     FROM villea04.AMBULATORY_MAPPING_VIEW) e 
  ON c.DEPARTMENT_ID = e.DEPARTMENT_ID) f
LEFT JOIN
...
LEFT JOIN
  (SELECT DEPARTMENT_ID, PROV_ID, SLOT_DATE, 
   ...
   GROUP BY DEPARTMENT_ID, PROV_ID, SLOT_DATE) h
...
LEFT JOIN
  (SELECT DEPARTMENT_ID, PROV_ID, SLOT_DATE, 
   ...
   AND SLOT_DATE BETWEEN TO_DATE('{date_1} 00:00:00', 'YYYY-MM-DD HH24:MI:SS') 
                     AND TO_DATE('{date_2} 23:59:59', 'YYYY-MM-DD HH24:MI:SS')
   GROUP BY DEPARTMENT_ID, PROV_ID, SLOT_DATE) i
...
LEFT JOIN
  (SELECT DEPARTMENT_ID, PROV_ID, SLOT_DATE, 
   ...
   AND SLOT_DATE BETWEEN TO_DATE('{date_1} 00:00:00', 'YYYY-MM-DD HH24:MI:SS') 
                     AND TO_DATE('{date_2} 23:59:59', 'YYYY-MM-DD HH24:MI:SS')
   GROUP BY DEPARTMENT_ID, PROV_ID, SLOT_DATE) j
...
LEFT JOIN
  (SELECT DEPARTMENT_ID, PROV_ID, SLOT_DATE, 
   ...
   AND SLOT_DATE BETWEEN TO_DATE('{date_1} 00:00:00', 'YYYY-MM-DD HH24:MI:SS') 
                     AND TO_DATE('{date_2} 23:59:59', 'YYYY-MM-DD HH24:MI:SS')
   GROUP BY DEPARTMENT_ID, PROV_ID, SLOT_DATE) k
