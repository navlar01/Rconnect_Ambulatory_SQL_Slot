...
SELECT f.*, g.holiday FROM
(SELECT e.CAMPUS, e.DEPARTMENT_ID, e.DEPARTMENT, e.CAMPUS_SPECIALTY, 
c.PROV_ID, TRIM(TRAILING FROM REGEXP_REPLACE(PROV_NM_WID, '\\[(.*?)\\]', '')) AS Provider, c.SLOT_DATE, c.SLOT_MONTH_YEAR, SLOT_DATE - (TO_CHAR(SLOT_DATE, 'D') - 1) AS APPT_WEEK, c.AVAILABLE_HOURS,
c.SCHEDULED_HOURS, c.ARRIVED_HOURS, TO_CHAR(c.SLOT_DATE, 'DY') AS APPT_DAY FROM(
    ...
    SELECT DISTINCT CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT_ID, DEPARTMENT FROM villea04.AMBULATORY_MAPPING_VIEW
